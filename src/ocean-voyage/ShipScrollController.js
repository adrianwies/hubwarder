...
export class ShipScrollController{
  ...
  setScrollProgress(progress){const reveal=Math.max(0,Math.min(1,progress/.055));const revealEase=reveal*reveal*(3-2*reveal);if(this.shipMaterial)this.shipMaterial.opacity=revealEase;const speed=Math.min(1,Math.abs(progress-this.previousProgress)*32);this.motionIntensity+=(speed-this.motionIntensity)*.22;this.previousProgress=progress;const center=this.centerTravelProgress??.18;let travelProgress;if(progress<.12){const entry=progress/.12;const easedEntry=entry*entry*(3-2*entry);travelProgress=gsap.utils.interpolate(0,center,easedEntry);}else{const x=Math.max(0,Math.min(1,(progress-.12)/.88));const delayed=Math.pow(x,1.65);const natural=delayed*delayed*(3-2*delayed);travelProgress=gsap.utils.interpolate(center,1,natural);}this.shipRoot.position.set(0,0,gsap.utils.interpolate(this.startZ,this.endZ,travelProgress));this.shipRoot.rotation.y=(progress-.5)*.018;}

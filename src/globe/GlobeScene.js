...
export class GlobeScene {
  constructor({container,debug=DEBUG}){if(!container)throw new Error('GlobeScene requiere un container');this.container=container;this.debug=debug;this.activeCountries=new Set(['Peru']);this.countryIndex=new Map();this.raf=0;this.visible=null;this.destroyed=false;this.timer=new THREE.Timer();this.timer.connect(document);}
  async init(){
    ...
    this.intersectionObserver=new IntersectionObserver(([entry])=>this.setActive(entry.isIntersecting),{rootMargin:'20% 0px'});this.intersectionObserver.observe(this.container);
  ...
  setActive(active){if(this.destroyed||this.visible===active)return;this.visible=active;if(active){this.timer.update();this.importFlow?.resume();this.render();}else{cancelAnimationFrame(this.raf);this.raf=0;this.importFlow?.pause();}}
  updatePeruLocation(){
  ...
  destroy(){this.destroyed=true;cancelAnimationFrame(this.raf);this.timer?.dispose();this.importFlow?.destroy();this.routes?.destroy();this.resizeObserver?.disconnect();this.intersectionObserver?.disconnect();this.controller?.destroy();this.scene?.traverse(o=>{o.geometry?.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>{if(!m)return;Object.values(m).forEach(v=>v?.isTexture&&v.dispose());m.dispose?.();});});this.renderer?.dispose();this.renderer?.forceContextLoss();this.renderer?.domElement.remove();}
}
